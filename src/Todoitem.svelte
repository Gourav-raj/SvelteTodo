...
<div class="todo-item">
 <div class="todo-item-left" tansition:fly="{{y: 20,duration:300}}">
    <input type="checkbox" bind:checked={completed} on:change={toggleComplete}>
    <div class="todo-item-label" class:completed={completed}>
    {title}</div>
    <div class="todo-item-date">{date}</div>
 </div>
 <!-- <button class="todo-item-update" on:click={updateTodoItem}>Update</button> -->
